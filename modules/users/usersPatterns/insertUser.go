...
type IInsertUser interface {
	...
	Result() (*users.UsersPassport, error)
}
...
func (f *userReq) Customer() (IInsertUser, error) {
	...
	query := `INSERT INTO users (username, password, email, role_id) VALUES ($1, $2, $3, 1) RETURNING "id";`
	if err := f.db.QueryRowContext(ctx, query, f.req.Username, f.req.Password, f.req.Email).Scan(&f.id); err != nil {
		switch err.Error() {
...
func (f *userReq) Result() (*users.UsersPassport, error) {
	query := `
      SELECT 
          json_build_object(
              'user', "t", 
